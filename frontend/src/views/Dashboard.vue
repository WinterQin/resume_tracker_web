          ...
          <el-row :gutter="12">
            ...
            <el-col :span="12">
              <el-card class="stat-card">
                ...
                <div class="stat-value">{{ stats.interview + stats.written }}</div>
              </el-card>
            </el-col>
          </el-row>
...
const fetchData = async () => {
  try {
    ...
    // 获取统计数据仍然需要所有申请记录
    const allApplicationsResponse = await applicationApi.getApplications()
    const allApplications = allApplicationsResponse.applications || []
    
    // 计算统计数据
    stats.total = allApplications.length
    stats.written = allApplications.filter(app => app.status === 'written').length
    stats.interview = allApplications.filter(app => app.status === 'interview').length
    stats.accepted = allApplications.filter(app => app.status === 'accepted').length
